...
function init() {
	...
	/**
	 * Sorts the `calamiteiten` array by theupdated_at property of the calamiteit (leverages UnderscoreJS _.sortBy function)
	 */
	...
	return _.map(rawData, function(item) {
		...
		return {
			...
			properties : {
				...
				calamiteit : item,
				editActions : [{
					title : isFavorite ? "- Favoriet" : "+ Favoriet",
					color : isFavorite ? "#C41230" : "#038BC8"
				}],
				canEdit : true
			},
			calamiteitName : {
				text : item.calamiteitTitel
			},
			calamiteitOmschrijving : {
				text : item.about.substring(0, 150) + "..."
			},
			calamiteitPhoto : {
				image : item.photo
			},
			calamiteitCategorie : {
				text : "Categorie: " + item.categorie
			},
			calamiteitLocatie : {
				text : " " + item.locatie
			},
			...
			calamiteitStatus : {
				text : item.start + " t/m " + item.eind
			},
			calamiteitVraag : {
				text : "Wilt u een enquête invullen i.v.m. " + "'" + item.calamiteitTitel + "'?"
			},

		};
	});
};
// call to action button that navigates to the vragenlijst window
...
var onBookmarkClick = function onClick(e) {
	 ...
	 * Appcelerator Analytics Call
	 */
	Ti.Analytics.featureEvent(Ti.Platform.osname + "." + title + ".favorieten.clicked");
